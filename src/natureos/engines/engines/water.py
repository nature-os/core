...
# These are simplified coefficients — full Penman-Monteith in later versions
WATER_DEMAND_MM = {
    WaterRegime.VERY_LOW: 200,      # Rainfall only after establishment
    WaterRegime.LOW: 400,           # Supplemental in dry months
    WaterRegime.MODERATE: 800,      # Regular irrigation
    WaterRegime.HIGH: 1400,         # Frequent irrigation
}

ESTABLISHMENT_YEARS = 2  # Years before species reaches stated water regime
...
class WaterBudget:
    ...
    def calculate(self, species_list: list[Species]) -> WaterBudgetResult:
        ...
        Assumes equal area distribution across species for initial estimation.
        """
        if not species_list:
            raise ValueError("Species list cannot be empty")

        # Total water demand from species requirements
        total_demand_mm = sum(
